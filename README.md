...
- [x] **Phase 1 — Pipeline backbone**: Greenhouse connector end-to-end (fetch → normalize → dedup → enrich → persist), Hangfire recurring runs, verified against the live Stripe board (594 jobs) via manual trigger; Lever connector added in Phase 5 (HighLevel board, ~90 jobs), each provider isolated behind `IJobConnector` with its own named HttpClient + resilience pipeline
